...
import { isPhraseCategory } from "@/lib/utils";
...
const TopicTabs: React.FC<TopicTabsProps> = ({ vocabulary, language }) => {
  ...
  // Group vocabulary by content type (words vs phrases)
  const groupedVocabulary = vocabulary.reduce(
    (acc, item) => {
      const isPhrase = isPhraseCategory(item.category);
      const contentType = isPhrase ? "phrases" : "words";
    ...
    () => [
      {
        id: "words",
        label: "Words",
        count: groupedVocabulary.words?.length || 0,
      },
      {
        id: "phrases",
        label: "Phrases",
  ...
  const defaultActiveTab = tabs.length > 0 ? tabs[0].id : "words";
